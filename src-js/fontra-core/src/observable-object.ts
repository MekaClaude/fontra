...
export class ObservableController<T extends {}> {
  ...
  _storageListener: ((event: StorageEvent) => void) | null = null;
  _mapKeyToObject: Record<string, string> | null = null;
  _setItemOnObject: ((key: any, value: string) => void) | null = null;
  ...
  destroy() {
    ...
    this._addSynchronizedItem = undefined;
...
function synchronizeWithLocalStorage<T extends {}>(
  ...
  controller._storageListener = (event) => {
    if (event.key !== null && event.key in mapKeyToObject) {
      // @ts-ignore
      setItemOnObject(mapKeyToObject[event.key], event.newValue);
    }
  };
  window.addEventListener("storage", controller._storageListener);

  controller._mapKeyToObject = mapKeyToObject;
  controller._setItemOnObject = setItemOnObject as (key: any, value: string) => void;
